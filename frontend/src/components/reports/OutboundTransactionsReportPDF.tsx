...
export const OutboundTransactionsReportPDF: React.FC<
  OutboundTransactionsReportPDFProps
> = ({ data, filters, reportMetadata }) => {
  // Calculate summary statistics
  const totalTransactions = data.length;
  const totalAmount = data.reduce(
    (sum, item) => sum + (Number(item.origin_amount) || 0),
    0
  );
  ...
  const currencies = [
    ...new Set(
      data.map((item) => item.origin_currency?.currency_code).filter(Boolean)
    ),
  ];
          ...
          <View style={styles.table}>
            ...
            {data.map((item, index) => (
              <View key={index} style={styles.tableRow}>
                ...
                <View style={styles.tableCol}>
                  <Text style={styles.tableCell}>
                    {formatToCurrency(Number(item.origin_amount) || 0)}
                  </Text>
                </View>
                <View style={styles.tableCol}>
                  <Text style={styles.tableCell}>
                ...
                <View style={styles.tableCol}>
                  <Text style={styles.tableCell}>
                    {item.origin_currency?.currency_code || "N/A"}
                  </Text>
                </View>
                <View style={styles.tableCol}>
                  <Text
