...
class OrganisationsService {
    async createOrganisation(organisationData, createdBy) {
        try {
            ...
            const organisation = await prisma.organisation.create({
                data: {
                    ...
                    status: client_1.OrganisationStatus.PENDING,
                },
                include: {
                    base_currency: {
                        select: {
    ...
    async toggleOrganisationStatus(id, status) {
        try {
            const organisation = await prisma.organisation.update({
                where: { id },
                data: { status },
    ...
    async getOrganisationStats() {
        try {
            const [total, active, inactive, pending, blocked, partners, agencies, customers,] = await Promise.all([
